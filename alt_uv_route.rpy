...
label alt_day4_uvao_aidpost: #беседуем с Виолой
    ...
    cs "Когда она в «Совенке» работать начала, эта легенда только появилась. Ей ее подопечные все уши прожужжать успели, теперь из-за каждого упоминания бесится. А потом как-то поутихли слухи понемногу."
...
label alt_day4_uvao_breakfast:
    ...
    "Детальный осмотр местности показал, что сесть я могу либо с рыжим дуэтом, к близости с которым всех предостерегал здравый смысл, либо с нашей доблестной девочкой-пулеметом восточного происхождения."
