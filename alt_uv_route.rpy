...
label alt_day4_uvao_breakfast:
    ...
    "Догнал меня звонкий голос Мику."
    hide mi with dspr
    ...
    "Озадаченный собственными рассуждениями, я перестал следить за дорогой и буквально врезался в Ольгу Дмитриевну."
...
label alt_day4_uvao_garlands_removal:
    ...
    show mt smile panama pioneer with dspr
    mt "Как ты догадался?"
    me "Если подумать, то создаётся впечатление, что я вам нужен только в качестве чернорабочего."
    ...
    "Я понял, куда она клонит. Кажется, поиск ответов придётся отложить."
    "В принципе я мог бы обойтись и без свидания с Жужелицей, но если хочется понять, что здесь происходит - придётся рисковать."
    mt "Как ты уже, наверное, понял, требуется убрать площадь, снять гирлянды и отнести их на склад."
    me "Вы всерьёз думаете, что один я справлюсь?"
    ...
    show mt normal panama pioneer with dspr
    mt "В общем, задание получил, вперёд выполнять."
    show mt smile panama pioneer with dspr
    "Она улыбнулась и твёрдой походкой направилась в сторону домика."
    hide mt with dissolve
    ...
    window show
    "Я постучал и дёрнул было ручку, но дверь оказалась заперта."
...
label alt_day4_uvao_library:
    ...
    window show
    "Шаг, другой.{w} Я ещё жив? И на меня даже не наорали? Удивительно!"
    "Уютное гнёздышко библиотекарши пустовало. Где же хранитель сего места?"
    ...
    mz "Я знаю, как меня зовут! Заруби себе на своём конопатом носу…"
    "Вот это подфартило! Впервые Электроник там, где нужен. Если он ещё потянет резину…"
...
label alt_day4_uvao_lunch:
    ...
    "Мысленно отмахнувшись от колкости, я взял с раздачи поднос и окинул взглядом столовую, выбирая, к кому бы из знакомых присоседиться."
    ...
    hide us
    with dissolve
    "В голове несколько раз эхом пронёсся демонический хохот, а я, рассудив, что обед в обществе сразу двух рыжих бандиток вряд ли принесёт какую-то пользу, а вот нервотрёпки избежать точно не удастся, огляделся в поисках более подходящей компании."
    menu:
        "Сесть с Леной":
            ...
            th "Какую ещё любезность?!{w} Что, моё альтер эго тоже сошло с ума?"
        ...
        "Сесть со Славей":
            ...
            if alt_day2_sl_guilty == 2:
                "А я даже ответил на её заботу, не подставив её позавчера с ключами перед вожаткой. Так что смело мог ожидать приятной беседы."
            elif alt_day3_dancing == 2:
                "Мне вспомнился вчерашний наш танец...{w=.4} и то, как я глупо сбежал в конце ловить фантомов с хвостами у столовой. Нужно было извиниться хотя бы за эту глупость."
            elif alt_day1_cofront_sl_dv == 2:
                "Только у неё за последнее время получилось сделать меня хоть чуточку небезразличным - я ведь пошёл из-за неё ругаться с Алисой в самый первый день. Похоже, наша активисточка неплохо влияла на меня."
...
label alt_day4_uvao_meet_Yulia_after_lunch:
    ...
    th "1928-й?"
    "Вместо ответа - многозначительный вздох разочарования. Кажется, я сморозил очередную глупость…"
    ...
    "Бледноватое лицо, густые, изрядно всклокоченные каштановые волосы, напряжённо расставленные уши."
    th "Что?! Уши?!"
    ...
    th "Цыц!"
    "Не так я представлял первый контакт с неизвестным мне гуманоидом."
    ...
    "Недовольно буркнул внутренний голос и выжидающе затих."
    stop music fadeout 3
    window hide
    ...
    if alt_uvao_D4_viola_morning:
        ...
        menu:
            ...
            "Сесть одному":
                ...
                "Проигнорировав гоготание ненавистного карманного собеседника, я забился в угол и принялся спешно поглощать еду."
    else:
...
label alt_day4_uvao_meet_Yulia_at_evening:
    ...
    "В кои-то веки спорить с альтер эго было решительно не о чем - в самом деле, из-за них я так толком ничего и не выяснил про эту ушастую."
...
# Д4 - Ходим по лагерю перед сном после второй встречи с Юлей
#
# Устанавливает флаг alt_uvao_true
label alt_day4_uvao_evening_business:
    ...
    if alt_uvao_D4_supper_cs:
        ...
        if alt_day1_sl_keys_took == 1:
            ...
            "Алиса принялась ковырять замок булавкой, как и в тот раз, я же вспомнил, в какое окно залезала Юля, и подошёл проверить."
            "Оказалось, если немного подёргать створку, шпингалет сам выходит из гнезда."
...
label alt_day5_uvao_tunnel:
    ...
    "Возразить мне было нечего. Действительно, стоя на месте много не узнаешь, и если уж я взялся искать ответы - упускать шанс получить хоть какую-то информацию было глупо."
    ...
    "Я не стал уточнять смысл намёка, а вместо этого повернулся к Юле, с интересом наблюдавшей за моими странными манипуляциями."
    me "Поможешь мне?"
    ...
    "А ведь когда-нибудь я привыкну к этим однообразным пошлым намёкам."
...
label alt_day5_uvao_bunker:
    ...
    uv "Спаривались."
    "Я вытаращил глаза."
    me "Чего-о?! {w=.4}Ольга Дмитриевна с физруком?!"
...
label alt_day5_uvao_hentai_scene:
    ...
    scene uv_new_hentai1_7dl with dissolve
    ...
    scene uv_new_hentai2_7dl with dissolve
    uv "Мааааауууу…"
    ...
    stop music fadeout 3
    "С приставучей голосиной говорить желания не было совершенно, однако она не оставляла попыток докопаться до меня."
...
label alt_day5_uvao_evade_hentai_scene:
    ...
    if alt_uvao_true:
        ...
        th "Не, ты не просто пошляк, ты нечто наглухо психанутое. Пойди проспись!"
        ...
        "…"
        dreamgirl "…"
        "Даже словоохотливая шизофрения не нашлась, как прокомментировать получившееся."
...
label alt_day5_uvao_hentai_sleep:
    ...
    hide bg int_sleep_hentai_office2_7dl
    window show
    ...
    hide mi with dissolve
    show bg int_sleep_hentai_office2_7dl behind prologue_dream with fade3
    hide bg int_sleep_hentai_office_7dl
...
label alt_day5_uvao_hentai_thinking:
    ...
    "Альтер эго отчего-то считало за должное поиздеваться надо мной, но я уже не слушал её, а лежал, просто наслаждаясь моментом."
...
label alt_day5_uvao_thinking:
    ...
    "Остапа понесло. Я сел на кровать и постарался отключиться от потока совершенно безумных идей и советов."
...
label alt_day5_uvao_mines_begin:
    ...
    if alt_uvao_D5_hentai:
        hide uv with dissolve
        "Проболтав без толку с внутренним пошляком, я не сразу заметил, что Юля куда-то пропала."
    else:
        hide uv with dissolve
    ...
    "Очередной подкол я решил попросту проигнорировать."
...
label alt_day5_uvao_from_mines_to_dinner:
    ...
    "Надо же, и от этого безумца бывает иногда польза."
...
label alt_day5_parking_back:
    ...
    window show
    show uv smile with dissolve
    "К моей радости, все инсинуации оказались беспочвенными - у консервов даже не кончился срок годности."
...
label alt_day5_capture_sh_together:
    ...
    "Надо думать, мой взгляд наглядно демонстрировал полное отсутствие мыслительных процессов, потому что она добавила:"
...
label alt_day5_uvao_evening:
    ...
    if (alt_uvao_D5_sh_mines):
        ...
        "Я не стал размениватся на препирательства и орать на вожатую тоже, хотя мне этого очень хотелось - и плевал я на последствия!"
        ...
        if not alt_uvao_true:
            ...
            "Проигнорировав надменное фырканье альтер эго, я перемешал содержимое тарелки в однородную массу и принялся за ужин."
            play music music_list["my_daily_life"] fadein 4
            ...
            th "В конце концов, я сегодня не только не пообедал толком, так ещё и полдник пропустил за всей этой беготнёй!{w} Непорядок, однако! Так и похудеть можно."
            dreamgirl "Можно подумать, последние несколько лет у тебя было исключительно пятиразовое питание ресторанного уровня, а здесь наоборот, голодом морить начали…"
            "Проигнорировав ехидное фырканье альтер эго, я перемешал содержимое тарелки в однородную массу и принялся за ужин."
            play music music_list["my_daily_life"] fadein 4
            ...
            "И, словно подслушав моего незабвенного пошляка, продолжила с ехидной ухмылкой:"
            show cs smile2 close with dspr
...
label alt_day5_uvao_ev_beach_event_direct:
    ...
    me "Эмм... Славя, почему ты вдруг это вспомнила?"
...
# Д5 - Ошибаемся и внезапно приходим к домику. Обалдеваем и остаёмся там.
...
label alt_day5_uvao_evening_headlong_already_here:
    ...
    if alt_uvao_D5_sh_mines:
        "Ольга недовольно дёрнула плечами и заторопилась было дальше, но, сделав несколько шагов, внезапно притормозила и снова обернулась."
    else:
