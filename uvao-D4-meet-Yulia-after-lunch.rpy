...
#
# используется флаг рассказа Слави про Генду за обедом uvao_D4_sl_genda_tale
#
...
#Придя, Семён приоткрывает окно, потому что домик прогрелся на солнце и читать умные книжки жарко
    ...
    "Под пакетом - колода игральных карт, перочинный нож, моток тонкого капронового шнура и, почему-то, мятый кусок свинца."
    th "Странный набор. Особенно свинец этот мне прямо-таки позарез нужен…"
    "Очаявшись понять причудливую логику моих «родителей», кто бы они ни были, я махнул на всё рукой и вытащил из-под подушки заветную «<книгу_что-то_там_про_Генду>»"
    ...
    if uvao_D4_sl_genda_tale:
#        th "Генда… Возглавлял… Соратник… Партия… Важнейшая роль…"
