...
#        hide mt
        ...
        cs "Присоединишься к поисковой партии, пионер, и проследишь, чтобы там всё прошло гладко, без ненужных контактов."
        me "Но…"
    ...
    mt "Что он там делал?"
    if alt_day4_uv_viola_evening:
        "Она покосилась на Эла и добавила, скорее для порядка:"
    ...
    me "А зачем руководителю технического кружка понадобились батарейки? По-моему логично."
    ...
    with dissolve
    if alt_day4_uv_viola_evening:
        me "Э-э… Судя по всему, четвёртым пойду я?"
