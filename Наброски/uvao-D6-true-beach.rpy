...
#
# используется флаг сопровождения в Д1 Славей от ворот до домика ОД
# используется флаг наблюдения за купанием Слави в озере в Д2 (alt_day2_date = 2)
...
#        ВНИМАНИЕ!
#         АХТУНГ!
# В ИГРЕ НЕ ЗАПУСКАТЬ - могут быть ошибки в командах RenPy
#
label alt_day6_true_beach:
# Медпункт снаружи, день
...
# Площадь, день
    ...
    "Впрочем, Шурик с этим проблем не испытвал, судя по вороватым взглядам, которые он бросал в сторону главных ворот лагеря, а заодно - и здания клубов."
    sh "Ну, это... Я пойду, наверное..."
    "Он непределённо помахал в воздухе рукой."
    ...
    "Осторожно покосившись назад полминуты спустя, я как раз успел увидеть очкарика, входящего здание клубов."
    ...
    "Я судорожно сжал кулаки в бессильной злобе. Кажется, Шизе в очередной раз удалось вывести меня из себя.{w} И ведь ничего не сделаешь с этой заразой..."
    "Кажется, на этот раз моё альтер-эго решило не добивать меня окончательно, потому что голос в моей голове прозвучал примиряюще:"
    dreamgirl "Ладно, хочешь хороший совет?"
    th "Не хочу!"
    "Буркнул я себе под нос, но Шизу это, разумеется, не остановило."
    ...
    dreamgirl "Может быть, хоть сегодня повезёт?{w} Да и вообще, не мешало бы тебе и в гигиенических целях ополоснуться, между нами говоря. Ходишь как бомж какой-нибудь, честное слово."
...
# Пляж, день
    ...
    show ul laugh swim far at fright with dissolve
    "Ребята постарше играли в пионербол - а во что же ещё? Разумеется, заправляла там Ульянка. Мне даже показалось, что она периодически умудрялась оказываться в нескольких местах одновременно."
    show dv normal swim at fleft
    show mi normal swim at left
    with dissolve
    "Немного поотдаль я увидел сидящих на песке и оживлённо о чём-то болтающих Алису и Мику."
    ...
    hide ul
    with dissolve
    "Лены нигде видно не было, что меня сейчас полностью устраивало. Разумеется, я ей был обязан за вчерашнее, но в последнее время каждая встреча с ней оборачивалась для меня какой-нибудь нервотрёпкой, а сейчас мне хотелось только одного - покоя."
# Пляж в Д3-волейбол гарантированно не посещали, плавок у ГГ нет - проверено. (ЛБ)
    ...
    "До меня наконец дошло, что пришло время для решительных мер, и я направился к вожатой."
    ...
    "От внезапного вмешательства Шизы я запнулся, но вожатая не стала ждать продолжения."
    mt "Давай, я попробую угадать. Ты решил искупаться, но у тебя нет плавок, верно?"
    me "Ну да, так и есть..."
    "Промямлил я в ответ. Усмехнувшись, Ольга открыла было рот, чтобы сказать что-то, но её перебили."
    ...
    hide sl with dspr
    "Выпалив всё это, Славя немедленно куда-то унеслась. Вожатая проводила её озадаченным взглядом и уставилась на меня."
    if alt_uvao_D6_CS_vetrov
        ...
        show mt angry swim with dissolve
        mt "Тебя с утра Виола искала. Ты где пропадал?"
        "Взглянув на её лицо, я с легкостью представил себе, как в и без того недовольном голосе вожатой появляются гневные ноты, и попытался уйти от ответа на опасный вопрос:"
    ...
    me "Виола упоминула, что я смогу остаться здесь, если... Ну, в общем, это от меня будет зависеть, останусь я {i}здесь{/i}, или нет."
    show mt surprise swim close with dspr
    "Ольга посмотрела на меня как-то растерянно и замялась." # замялась? хммм.
    ...
    "Как обычно, Шизу было не понять, да я не очень-то и старался."
    ...
    "От очередного раунда бессмысленных препирательств с Шизой меня спасла запыхавшаяся Славя."
    show sl normal pioneer with dissolve
    sl "Вот, держи. Думаю, подойдут по размеру."
    "Она сунула мне в руки нечто чёрное, оказавшееся обычными мужскими плавками."
    ...
    if (alt_day2_date = 2)
        ...
        "Я невольно улыбнулся и от души поблагодарил девочку:"
...
# blink или ещё какое короткое затемнение.
    ...
    "Кажется, купание пошло мне на пользу. Вместе с грязью я словно бы смыл и всю накопившуюся за эти шесть безумных дней усталость.{w} Сейчас завтрашний день уже не казался мне таким пугающим."
    ...
    th "В сущности, Ольга мне дала неплохой совет - попытаться понять, чего мне хочется на смом деле. Неужели я в собственных желаниях не смогу разобраться?"
    ...
    show sl smile swim close at left with dissolve
    "Рядом сидела, мило улыбаясь, раскрасневшаяся после купания Славя. Мокрые волосы, на коже блестят невытертые капельки воды. Словно зачарованный, я проследил, как одна из них скатилась с ключицы вниз, на плотно обтянутую купальником высокую..."
    ...
    show sl laugh swim close at left with dissolve
    sl "Да говорю же - ерунда. В конце концов, не голышом же тебе было купаться, при всех-то!"
    show sl smile swim close at left with dissolve
    "Сделав ударение на последнем слове, она вновь одарила меня лукавым взглядом и, развернув невесть откуда взявшееся полтонеце, принялась вытираться."
    "Словно зачарованный, я смотрел, как она неспеша обтёрла плечи, потом закинула полотенце за спину, гибко прогнувшись вперёд..."
    hide sl with dissolve
    "Я отвёл вгляд, чувствуя, что щёки начинают пылают. В голове раздался смешок:"
...
# возможно, побольше комментариев Шизы про Мику! -ЛБ
    ...
    "С минуту мне казалось, что импровизированная мантра начала мне помогать."
    "Я сосредоточенно пялился на воду, даже Славя рядом притихла, перестав шуршать полотенцем - наверное, закончила вытираться."
    th "Нет-нет, никаких полотенец. Тело моё свежо, разум мой расслаблен... Тфу ты! Наоборот. Тело моё расслаблено, разум свеж. Речные волны..."
    ...
    show dv normal swim far at cleft with flash
    ...
    "Поймав мой взгляд, Алиса на секунду нахмурилась было, но тут же самодовольно ухмылнулась и, не отводя глаз, повернулась вполоборота."
    # heartbeat 
    ...
    with vpunch
    $ renpy.music.set_volume(1.0, delay=1, channel='sound_loop') # проверить, нельзя ли упростить до volume() -ЛБ
    ...
    th "Чёрт!"
    "Плавки внезапно оказались нестерпимо тесны, и со всей очевидностью мне стало ясно, что ещё несколько секунд - и я погиб. У меня даже полотенца нет, прикрыться..."
    stop sound_loop
    play sound sfx_soccer_ball_kick
    hide dv #прячем без перехода
    with hpunch
    with flash_red
    "Что-то с силой ударило меня в голову, так что искры из газ посыпались... И я обнаружил себя уже лежащим на боку. Сквозь шум в ушах до меня донёсся далёкий крик:"
    sl "{b}УЛЬЯНААААА!!!{/b}"
    ...
    show sl scared swim close with dissolve
    sl "Семён, как ты?"
    ...
    show mt rage swim far at fright
    show us dontlike swim far at cright
    show sl normal swim close at left with dissolve
    with dissolve
    ...
    show sl serious swim close at left with dissolve
    sl "Представляешь, Ульяна взяла и запулила в тебя мячом со всей дури!"
    "Я сопоставил относительно небольшой вес волейбольного меча с силой обрушившегося на меня удара и посмотрел на Ульяну с невольным уважением."
    ...
    show sl angry swim close at left with dissolve
    ...
    show sl serious swim close at left with dissolve
    "Несмотря на редкостное единодушие, возникшее у нас с Шизой по данному вопросу, скандалить мне по-прежнему не хотелось."
    show mt angry swim far at fright
    show us dontlike swim far at cright
    with dissolve
    "Я снова посмотрел в сторону Ульяны. Ольга наконец закончила ей выговаривать - наверное, выдохлась."
    "Та тоже больше не огрызалась, а молча стояла, полуотвернувшись от вожатой и надувшись как мышь на крупу."
    ...
    show mt surprise swim far at fright
    show us angry swim far at cright
    with dissolve
...
# Пристань, день
    ...
    if alt_day1_sl_conv
        ...
        "Как и в первый день, я разулся и уселся на мостки, свесив ноги в воду."
    "Подозрительно помалкивавшее последние несколько минут подсознание вдруг подало голос:"
    dreamgirl "Я тебя прямо-таки не узнаю. Где твоя здоровая мизантропия? Где человек человеку люпус эст? Где око за око? Вроде бы тебе по голове не так уж сильно заехали, в чём дело?"
    ...
    "Впрочем, спору нет, прилетевший мяч напрочь выбил на какое-то всремя все гормоны из головы, да и из остального тела тоже."
    dreamgirl "Вот то-то же!"
    play sound eat_horn fadein 3
    "Итог этой поучительно беседе подвёл горн."
    stop sound fadeout 2
    "Я поднялся, неспеша обулся и вразвалочку пошёл к столовой."    
