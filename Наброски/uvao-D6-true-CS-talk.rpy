...
label alt_day6_true_SH_exam:
    ...
    dreamgirl "Просто ты ей нужен более-менее соображающий, а не пускающий слюни. А Шурик наверняка от нее с утра наретпелся, поэтому и зашуганный такой"
    ...
    sh "А… психически?"
    cs "А голова - предмет темный… Впрочем, будет полезно, если ты расскажешь, чем ты вчера занимался и что с тобой произошло."
    "Шурик задумался."
    ...
    th "Что-то я сомневаюсь, что он за аппаратурой туда поперся…"
    cs "Понимаю. А дальше?"
    ...
    cs "Не исключено…"
    "Она достала из шкафа растрепанную картонную папку, подписанную \"Трофимов А.\", пролистала страницы, сплошь покрытые врачебными каракулями…"
    ...
    cs "И поменьше торчи в вашем клубе. У вас там от канифоли не продохнешь, и здоровому-то поплохеет, а тебе сейчас это вообще противопоказано."
