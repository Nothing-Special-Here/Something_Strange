...
# Здесь нужна рефлексия о том, как мало успелось и хочется ли возвращаться назад.
    ...
    dreamgirl "Ты прав. {w=1.0}Почти прав. {w=1.0}Тебе и здесь совсем нетрудно будет отгородиться ото всех стеной безразличия, ты неплохо освоил это умение, и тебя рано или поздно оставят в покое."
...
# Чувствуем нарастающую панику - обратно тоскливо, здесь - ссыкотно
    ...
    show cs normal3 close with dissolve
    cs "В общем-то да. С момента начала регулярных исследований таких долговременных проколов было зафиксировано всего четыре."
    ...
    dreamgirl "Не бойся, чувак, ты совершенно здоров. Небольшое раздвоение личности - не в счёт.{w} Тётя просто пошутила."
    "На моё счастье, Виола не заметила, как я отвлёкся, и продолжала говорить:"
    show cs normal2 close with dissolve
    cs "…происходит с другими — ты видел вчера."
    "Мне стоило некоторых усилий снова поймать нить разговора. Дабы показать, что слушаю, я скорчил глубоко сочувствующее выражение лица:"
    ...
    if alt_uvao_D6_CS_vetrov: # длинный разговор за чаем
        ...
        cs "Более-менее ясно, что визуализация является частью прокола. Большинство попаданцев упоминает о том, что в процессе перехода общались с некоей личностью, причём все они описывают её как нейтрально или даже дружелюбно настроенную."
        ...
        cs "Ну вот видишь. Что-то наподобие проводника между реальностями.{w} Другое дело, что в обычных проколах её проявления этим и ограничиваются. Скорее всего, там она и существует только для субъекта переноса."
        ...
        cs "Вероятно, здесь она начинает выполнять ещё и защитную функцию - не допускает перемещения случайных людей между реальностями."
        cs "Хотя, вообще, это уже из разряда домыслов… Но зато неплохо объясняет, почему таких спонтанных перемещений посторонних людей не происходит, хотя мощности долгосрочных аномальных переходов…"
        show cs smile3 close with dissolve
        "Виола оборвала сама себя, виновато улыбнувшись."
        cs "Извини, что-то я слегка увлеклась.{w} Объясню попроще: все наблюдавшиеся долгосрочные проколы очень быстро становились достаточно, скажем так, широкими, чтобы в них могло затягивать случайно оказавшихся рядом людей, в том числе и {i}отсюда{/i}."
        show cs normal3 close with dissolve
        cs "Теперь факт первый. Для перемещения между реальностями необходимо как-то с взаимодействовать с проколом."
        cs "Факт второй. Ни одного случая таких перемещений не зафиксировано, зато большинство контактов с аномальной визуализацией заканчивались потерей памяти, либо психическими расстройствами различной тяжести."
        cs "Можно сделать вывод, что для аномальная визуализация принимает меры для предотвращения переноса, делая невозможным взаимодействие субъекта со структурой прокола."
        me "Э-э-э…"
        show cs important close with dissolve
        cs "Говоря совсем простым языком - взаимодействия между структурой прокола и разумом, сознанием человека не произойдёт, если нечему будет взаимодействовать. {w}Вот это-то аномальная визуализация и обеспечивает, причём вполне эффективно, делая человека непригодным к взаимодействию."
        show cs normal3 close with dissolve
        me "Так значит…?"
        cs "Да. И к сожалению, каждый раз существует риск, что защита окажется {i}слишком{/i} эффективной."
        cs "Как жаль, что в семьдесят четвёртом мы ещё об этом не знали…"
        me "А что тогда было?"
        "Виола помрачнела, и я пожалел, что своим вопросом пробудил в ней какие-то неприятные воспоминания. Я уже было хотел дать задний ход, как Виола наконец ответила:"
        ...
        "Виола говорила спокойно, однако я не мог не заметить, что тема эта ей неприятна. Поэтому я решил помочь ей, направив разговор в сторону:"
        me "Эффект Ветрова? А что это?"
        show cs normal close with dissolve
        cs "А это, судя по всему, как раз проявление защитного механизма аномалии на полную мощность. Назвали, кстати, по фамилии попаданца из того прокола, где это произошло."
        show cs normal2 close with dissolve
        cs "Когда он впервые проявился, всё было примерно так же, как и у нас здесь. Долгосрочный аномальный прокол с выходом в районе турбазы, в какой-то момент произошёл наконец перенос субъекта, началась адаптация."
        cs "Потом появилось что-то вроде того, что ты назвал Юлей. Как я подозреваю, кому-то из начальства наград захотелось, но это между нами.{w} Так или иначе, бросили группу армейского спецназа на захват."
        show cs normal close with dissolve
        cs "Вскоре после этого местность… закрылась, скажем так. Наверное, со стороны это даже забавно должно было выглядеть - турбазу накрыло эдаким куском тумана.{w} Входишь в него — ничего особенного не происходит, пара минут в густом тумане и выходишь с другой стороны. Но внутри ни зданий, ни растений. Ни людей."
        show cs normal3 close with dissolve
        "Виола замолчала, и я не удержался от вопроса:"
        me "Ну так а дальше-то что?"
        show cs doubt3 close with dspr
        cs "А что дальше? Оцепили, стали ждать. На четвёртый день двое из тумана этого вышли - всё, что от группы захвата осталось."
        ...
        "Мы помолчали."
        me "Хорошо, я понял. Люди, оказавшиеся на грани перехода, как-то взаимодействуют с аномалией и она их… нейтрализует, если они не подходят. Но что насчёт меня? Я не помню, чтобы я с ней как-то общался в своём мире."
        ...
        "Я собирался расспросить Виолу обо всем этом поподробнее, но тут в дверь постучали."
