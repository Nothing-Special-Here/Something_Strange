...
label alt_day6_true_CS_talk_short:
    ...
    "Виола кивнула на стоящий на письменном столе странноватого вида металлический чайник, суда по проводу - электрический."
...
# Чтобы отвлечься:
    ...
    if semen_panique: # Паника возле автобуса
        ...
        dreamgirl "Все познается в сравнении. Возможно, другие вели себя еще хуже."
    ...
    cs "Видишь ли, пионер, проколы бывают двух видов. Обычный прокол открыт примерно неделю, плюс-минус. О многих из них мы узнаем пост-фактум, когда попаданец возникает в нашем мире."
    ...
    cs "Но иногда, по непонятным причинам, прокол становится долговременным. И вот в них-то и заводятся подобные… э-э-э… существа. Мы считаем, что это одно из проявлений самого прокола."
    ...
    cs "Да. За последние семьдесят лет таких долговеременных прокола было всего четыре. И обращаться с визуализациями следует с большой осторожностью."
    me "Почему вы тогда были так уверены, что со мной ничего не произойдёт?"
    cs "Общение с визу… с Юлей опасно для всех, кроме тебя. На тебя у неё привязка своего рода, поэтому ты ничего не ощущаешь." 
    ...
    if alt_uvao_D6_CS_vetrov: # длинный разговор за чаем
        ...
        me "Значит...?"
        cs "Да. И к сожалению, это иногда происходит дорогой ценой. Полагаю, вы сегодня успели весьма и весьма вовремя."
        ...
        # внезапный страшный звук сюда! нагнетем!
        th "Серьёзно?"
        $ renpy.pause(2.0)
        dreamgirl "Ха, повёлся."
        cs "Ну, даже догадатьтся не попробуешь кто?"
        dreamgirl "Да, интересно, это что за покемон?"
        "Мне почему-то оказалось, что ответ лежит на поверхности, что он совсем очевиден. Однако, в голову ничего не лезло."
