...
label alt_day6_uvao_isolator_house:
    ...
    # TODO: Больше инфы про Шурика, про эффект чуть-чуть, и про то, что во всем виновата Юля
    ...
    th "Что за херня здесь происходит?!" with hpunch
    # Сидим, накручиваем себя дальше. Склоняемся к выбору - Юля или Виола с ОД.
    # Внезапно Юля в окне. "Ты там котлеты ешь? А мне?". Семен пугается от неожиданности.
    # Если выбрал Юлю - успокаивается и просит принести ключи. Если выбрал Контору - держимся подальше, Юля обиженнная уходит.