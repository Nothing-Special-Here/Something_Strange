...
# используется флаг Альтернативного Д1 alt_day1_random_val
...
#    
    ...
    menu:
        "Сесть с Алисой":
            ...
            "Кажется, сейчас даже возможные подколки Алисы будут лучше, чем безудержное внимание активистки."
            me "Свободно, подруга?"
            ...
            if alt_uvao_true:
                ...
                show dv upset pioneer close with dissolve
                dreamgirl "Ты что мелешь? Тебе давно тарелку на голову не надевали?"
                "Прежде чем Алиса успела всерьёз обидеться, я поторопился выправить ситуацию, наклонившись к ней и доверительно сообщил:"
            ...
            "Странно, но судя по вновь проступившему на щеках Алисы румянцу и скользнувшей на её губы самодовольной ухмылке, мой неуклюжий \"комплимент\" попал в цель."
            ...
            if alt_uvao_D4_lunch_un:
                ...
                dv "Я её хорошо знаю...{w=0.5} Ну, вернее, как хорошо...{w=1.0} Лучше других здесь, во всяком случае. Мы живём недалеко и не первый год уже дружим."
                dv "Она всегда была... {w=0.5}немного замкнутая. Но сейчас совсем в себя ушла, молчит всё время."
                show dv angry pioneer close with dissolve
            ...
            if (alt_uvao_D4_lunch_un or alt_uvao_D5_evening_dv_un):
                ...
                if (alt_uvao_D4_lunch_un and alt_uvao_D5_evening_dv_un and alt_uvao_D6_CS_vetrov):
                    ...
                    nvl clear
                    dreamgirl "Мне тоже кажется, что кое-что упускаешь. Так и быть, дам тебе подсказку."
                    ...
                    nvl clear
                    th "Действительно, а что если Лена столкнулась с хвостатой, той что-то не понравилось, ну и... Шурик после {i}обработки{/i} начал голоса слышать, а Лена по-своему могла отреагировать."
                    th "Впрочем, можно попробовать порасспросить кого-нибудь, кто её хорошо знает. Вот только вопрос - кого? Она же стесняша нелюдимая."
                ...
                window show
                dv "Эй, на барж{b}е{/b}?!"
                "Вздрогнув от звонкого щелка пальцами под самым носом, я понял, что уже пару минут сижу с недонесённой до рта вилкой и тупо пялюсь куда-то Алисе за спину."
            ...
            mt "Ребята, напоминаю вам ещё раз, что сегодня после полдника состоится итоговый концерт! Или вместо полдника. Или параллельно полднику... Не помню точно. Но обещаю, будет интересно, так что не опаздывайте!" #ЛБ: после полдника ведь?
            $ renpy.music.set_volume(1.0, delay=3, channel='ambience')
            mt "Явка, кстати, обязательна для всех."
            ...
            if (alt_uvao_D4_lunch_un and alt_uvao_D5_evening_dv_un):
                ...
                menu:
                    ...
                    "Не надо суеты!":
                        ...
                        th "Через сутки мы всё равно, так или иначе, но разбежимся в разные стороны, лучше уж просто подождать."
                        ...
                        "Шолос Шизы показался мне укоризненным, и я немедленно огрызнулся:"
                        ...
                        "Я представил себе, как лезу к Лене с объяснениями, а та в ответ смотрит на меня, как на последнего идиота, и меня бросило в жар."
                        th "Нафиг!"
