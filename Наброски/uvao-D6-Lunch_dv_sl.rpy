    ...
    menu:
        "Сесть с Алисой":
            ...
            if alt_uvao_D4_lunch_un:
                ...
                dv "Я её хорошо знаю...{w=0.5} Ну, вернее, как хорошо...{w=1.0} Лучше других, во всяком случае. Мы живём недалеко и не первый год уже дружим."
...
#                "Поперхнувшись пловом, я отчаянно закашлялся и уставился на Алису сквозь выступившие слёзы."
            ...
            if (alt_uvao_D4_lunch_un or alt_uvao_D5_evening_dv_un):
                ...
                th "Внешность у меня заурядная, интеллектом-шутками блеснуть у меня возможности никакой не было. Да если бы даже и была..."
                ...
                if (alt_uvao_D4_lunch_un and alt_uvao_D5_evening_dv_un and alt_uvao_D6_CS_vetrov):
                    ...
                    if alt_day1_random_val == 1:
                        # Альтернативный Д1, Лена участвовала в погоне за Юлей
                        # ЛБ: Необходимо будет уточнить подробности - вспоминал ли ГГ в Д2,Д3 (а то и в Д4-Д5, чем чёрт не шутит?) о попытке отжать телефон
                        # ЛБ: И вообще, вспомнит ли ГГ о той беготне?
                        th "Кстати, да ведь в первый же день, когда Юля у меня смартфон попыталась отжать - Лена за ней вместе со мной гонялась, вот тогда могла и схлопотать, например. {w}То-то когда она меня у памятника догнала наконец, то еле на ногах держалась."
                    th "Впрочем, можно попробовать порасспросить кого-нибудь, кто её хорошо знает. Вот только вопрос - кого? Она же нелюдимая."
                    dreamgirl "Ты мог и не заметить, но прямо перед тобой её подруга сидит - её и расспрашивай."
                    th "Нет уж, дудки! Я жить хочу!"
                    dreamgirl "Ну так с соседкой Лены поговори, с японкой этой! Подруги они или нет, но Лену она как мимнимум дважды в день видит. Вполне могли перед сном о своём, девчоночьем, поболтать-посплетничать."                        
                else:
                ...
                # Текст не нужен.
                pass
            "Не сговариваясь, мы с Алисой по-быстрому прикончили свои порции, залпом выпили неизбежный компот и поспешили освободить помещение столовой."
            window hide
