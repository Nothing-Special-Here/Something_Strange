label alt_day5_uv_bunker:
    ...
    $ alt_day5_sleep = False # Сон
    $ true_flag = False # TODO: Найти название этого флага и ткнуть сюда
    ...
    menu:
        "Сменить тему":
            "Семён как бог меняет тему." #TODO: Текстик переписать в код https://docs.google.com/document/d/12x2m4SJ-Uh37bbK3s2PfsB14Fb-3ak2bFTzCkN-WtvQ/edit
            menu:
                "Продолжить расспросы":
                    "Ему интересно, идём дальше. Ложный рут."
                "Задуматься":
                    "Ему не интересно, забил. Идёт в шахты, никого не видит - обед. Нейтрал."
        "Узнать поподробней":
            $ alt_day5_uv_hentai = True
            jump alt_day5_uv_hentai

label alt_day5_uv_hentai:
    ...
    "С Шизой говорить желания не было, однако она не оставляла попыток докопаться до меня."
    dreamgirl "Видел бы ты себя со стороны, кошачий угодник."
    menu:
        "Проигнорировать":
            $ alt_day5_sleep = True
            "Шиза всё-ещё старалась привлечь моё внимание, однако ни сил ни желания разговаривать с ним не было, и я, сам этого не замечая, провалился в сон."
            ...
            #TODO: Сон с кошками - https://docs.google.com/document/d/121jxSZ1V8IlG2P-ExV9-9iHwkBX-jq1mDb9i2VFz1Z4/edit
        ...
        "Побеседовать с Шизой":
            ...
            "Шиза считала за должное поиздеваться надо мной, но я уже не слушал его, а лежал, просто наслаждаясь моментом."
    ...
    jump alt_day5_uv_mines_begin

#Сама шахта, начиная от похода к ней.
#TODO: Переодевалки в хентай кинуть, чтобы начинались шахты со слов: Мы вышли из двери, бла-бла.
label alt_day5_uv_mines_begin:
    ...
    #Наверное лучше развилку сделать тут, и попадаешь на тот фон, где в оригинале повязку шурика находят.
    #Если шурика тут нет - она говорит налево, и идёт к двери. А если он там то видит его где-то в темноте, ну или как-то так.
    ...
    if alt_day5_sleep:
        ...
        jump alt_day5_uv_mines_sh
    else:
        "Видим дверь, идём на обед"
        jump alt_day5_uv_mines_dinner
    
    
label alt_day5_uv_mines_dinner:
