...
label alt_day5_uvao_evening_dv_un:
    ...
    dreamgirl "Это {i}там{/i} не бывает, точно так же, как и девочек с кошачьими ушами и хвостом, кстати говоря. Только вот ты-то не там, а {i}здесь{/i}."
    "Занятые разговором девочки пока что не замечали моего появления, но продолжаться бесконечно это не могло."
    th "\"Там\" я, или не \"там\", но у меня такое ощущение, что лучше бы мне уйти подобру-поздорову, пока они меня не увидели. Решат ещё, что подслушиваю."
    if alt_uvao_true:
        ...
        th "Тем более, она ещё вчера достаточно странно себя вела. Не вышло бы сейчас какого-нибудь продолжения."
    else:
        pass
