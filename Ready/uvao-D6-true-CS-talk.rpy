...
# Чувствуем нарастающую панику - обратно тоскливо, здесь - ссыкотно
    ...
    me "Да… Очень похоже на вид из моего окна.{w} Так получается, что это первая фотография… как её… визуализации?"
    show cs normal3 close with dissolve
    ...
    "И, как довольно быстро выяснилось, взаимодействовать с визуализациями следует с большой осторожностью.{w} Были… гм… неприятные инциденты."
    me "Тогда почему Вы были так уверены, что со мной ничего не произойдёт?"
    cs "Видишь ли, общение с визу… с Юлей потенциально опасно для всех, кроме тебя. На тебя, как на субъекта переноса, у неё своего рода привязка, поэтому ты её воздействия не ощущаешь."
    show cs normal3 close with dissolve
    ...
    if alt_uvao_D6_CS_vetrov: # длинный разговор за чаем
        ...
        cs "Так вот, более-менее ясно, что визуализация является частью прокола."
...
# девочка - виденье.
        ...
        "Мучительно путаясь в словах, я, как мог, рассказал Виоле о смутном видении девочки, которая спрашивала меня, пойду ли я с ней."
        ...
        cs "А вот в случае аномального прокола, судя по всему, визуализация становится довольно самостоятельной сущностью, хотя и связанной по-прежнему с точкой прокола."
        ...
        cs "Из этого они делают вывод, что это именно визуализация принимает меры для предотвращения переноса, делая невозможным взаимодействие субъекта со структурой прокола."
        me "Э-э-э…"
        show cs important close with dissolve
        cs "Говоря совсем простым языком - взаимодействия между структурой прокола и разумом, сознанием человека не произойдёт, если нечему будет взаимодействовать. {w}Вот это-то визуализация и обеспечивает, причём вполне эффективно, делая человека непригодным к взаимодействию."
        show cs normal3 close with dissolve
        cs "Впрочем, повторюсь, это всего лишь гипотеза, причём весьма сомнительная, на мой взгляд. С куда большей вероятностью всё это может быть попросту механизмом самозащиты самой визуализации."
        ...
        cs "Решила Юлия Валерьевна в обход всех инструкций контакт с визуализацией наладить. {w}Наладила… Спасибо, эффектом Ветрова лагерь не накрыло."
        ...
        show cs normal close with dissolve
        cs "А это, судя по всему, как раз проявление защитного механизма визуализации на полную мощность, что бы там она ни защищала при этом - себя или аномалию."
        ...
        show cs smile2 close with dspr
        cs "Потом внезапно установился контакт и началось взаимодействие субъекта с визуализацией. {w}Опять-таки, похоже на нашу ситуацию, правда?"
        ...
        cs "До работающих на аномальных проколах довели краткую инструкцию о недопустимости форсирования контактов с визуализациями - без объяснений и подробностей."
        ...
        cs "А так она решила рискнуть. На тот момент аномалия существовала уже... {w=0.8}да, восемнадцать лет, никаких серьёзных происшествий не было зафиксировано. Визуализацию замечали периодически, но какой-либо агрессии она не проявляла."
        ...
        cs "Но никакого коллапса не произошло! Прокол по-прежнему оставался открытым, хотя визуализация долгое время после этого не появлялась."
        ...
        me "И что, вот так вот всё просто? {w}Я имею в виду, Вы так легко об этом говорите - проколы, переходы, визуализации…"
