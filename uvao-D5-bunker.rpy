...
#
    ...
    "Юля замолчала, так что с минуту я просто стоял на месте столбом."
    show uv normal with dspr
    "Постепенно зрение адаптировалось, и я понял, что мрак не был вовсе беспросветным.{w} Мы явно стояли в начале какого-то коридора или тоннеля, и издалека доносился слабый свет."
    me "Юля, а куда коридор ведёт?"
    uv "Никуда. Там дверь, большая железная. Не открывается."
    me "Чем на месте стоять, пойдём, посмотрим? Вдруг вместе получится её открыть?"
    show uv grin with dspr
    uv "Пойдём, попробуем открыть!{w} Узнаем, что там!"
    "Юля схватила меня за руку и потащила куда-то в сторону источника света."
    ...
    show uv normal with dspr
    th "Интересно, а куда этот тоннель ведёт? Спуск в бомбоубежище?"
    ...
    "В желтоватом свете пыльной лампы хорошо были видно, что стенам уже не один десяток лет, но время практически не коснулось их - ни трещин, ни потёков воды."
...
#    "Почему-то я решил не выдаваться в сравнительный анализ коитуса грызунов и людей."
    ...
    if alt_uvao_true:
        ...
        "Шкафчики с разными необходимыми припасами. Противогазы, аптечки с описью, инструменты… Но, кажется, ничего такого, что могло бы пригодиться." 
...
#
    ...
    "Я уже собрался выскочить на подмогу, но… Непоняно как спасатели повалили нашего Рэмбо и старательно крутили ему руки."
    ...
    dreamgirl "Вот что значит советская педагогика! А если б ногами - вообще был бы как щелковый!"
