...
#    "Почему-то я решил не выдаваться в сравнительный анализ коитуса грызунов и людей."
    ...
    if alt_uvao_true:
        ...
        "Я открыл рюкзак и начал искать в нём подарок \"родителей\". Под руку попадался всякий хлам, который в данной ситуации мне был не нужен, типа лески или ножа, но одна находка была очень кстати. Спички!"
        th "А что, вдруг пригодятся?"
        "Не долго думая я закинул их в карман шорт, и стал дальше искать в рюкзаке своё спасение. {w}Наконец конфеты были найдены."
...
#
    ...
    # Не знаю, какую лучше из песен взять
...
#
    ...
    # можно было бы зациклить этот кусок, чтобы он игрался параллельно тексту - зажечь спичку, посмотреть на перекресток, потушить. Кажись что-то такое видел в документации
    # Да вроде всё норм. - Ravsii
    "Так мы и ходили. То направо, то налево. Я иногда зажигал спички, но все перекрестки были похожи как близнецы."
    ...
    "Шиза снова не упустила повод меня подколоть, но я решил проигнорировать этот выпад." #TODO: Изменить фразу
    if alt_uvao_D5_sh_mines:
        ...
        # Мб спрятать это под условие (не шибко силён в погромировании...в коде автора вступившие прописываются под $ alt_day2_club_join_cyb = True) для тех, кто не вступил в клуб кибенетиков во время обхода?
        # По бабблу ничего нет - так что я думаю не стоит. Можно добавить несколько, не на что не влияющих, строк, которые будут видны только если ты в клубе. А если ты чисто про друга - это уже не первый её подкол, так что, кмк, друг пусть висит всегда. - Ravsii
...
#
    ...
    with dissolve
    window show
    "Брошеные инструменты, какие-то разбитые ящики. Рельсы заворачивали вправо, и у стены лежала опрокинутая вагонетка."
    ...
    scene cg d4_sh_sit
    window show
    "Шурик действително был там. С фонариком, с какой-то сумкой через плечо - он явно подготовился к походу в катакомбы."
    ...
    th "Да хрен его потом тут найдешь, в шахтах-то. Надо как-то увести его отсюда. Но как?"
    ...
    "Шурик вздрогнул, вкочил и вжался в стену."
    sh "Семён?… Это ты?…"
    ...
    me "Да кто она-то?"
    th "Он что, Юлю сюда пришел искать?!"
    ...
    sh "Ты…{w} мне…" 
    show sh angry bar3 close with dspr #Палка в замахе
    extend " всё…{w} расскажешь…"
    dreamgirl "Берегись!"
    window hide
    show cg d4_sh_stay
    window show
    "Он ринулся вперед, целя дубиной мне в голову."
    "Я рефлекторно отпрыгнул назад,{w} но оступился на камнях и рухнул на спину." with vpunch
    window hide
    scene bg int_mine_coalface
    show sh angry bar3 far at cright #Палка опущена
    show uv rage close at cleft
    window show    
    "Внезапно, с каким-то странным яростным мявом из-за камней выпрыгнула Юля."
    ...
    dreamgirl "Тебе железяки в голову показалось мало? Охота еще лопатой по шее получить?"
    th "Ну блин, явно же не в себе человек! Если его бросить - навернется, сломает себе что-нибудь и так и сгинет.{w} А потом сюда пригонят спасателей, они тут все перероют… И Юлю уже точно найдут!"
    me "Юля. Ты не знаешь, куда он побежал?"
    uv "Обратно туда. Откуда мы пришли."
    me "Проведешь? Надо его отсюда выгнать, вернуть заблудшую душу в коллектив, так сказать."
    ...
    "Я подобрал фонарик Шурика - трофей, взятый в бою! Теперь не придется спотыкаться в темноте и жечь спички."
    ...
    me "Идём!"
    # надо менять перекрестки под звуки топота и дыхания, с раскачиванием влево-вправо.
    # TODO: Доделать спрайты и фон
    "Идти с фонарем было гораздо быстрее, чем спотыкаться в полной темноте. Почти бегом мы пересекли несколько перекрестков - каждый раз Юля без тени сомнения выбирала направление."
    th "Интересно, это она на слух определяет, куда идти, или следы замечает?"
    "Кажется, мы его нагоняли - мне иногда чудился какой-то свет впереди. Все-таки у него нашелся запасной фонарь."
    # дыра с насыпью
    ...
    "Нас обоих разбирал азарт погони. Я с трудом представлял, что я буду делать, когда догоню его, но это все казалось неважным - впереди добыча! Уходит!"
    # тоннель
    "Наверху оказался тоннель - похоже тот самый, в который вел люк из старого корпуса."
    me "Это тот, первый тоннель?"
    uv "Да. Лестница там, а он побежал туда. Там тоже выход есть."
    "Она махнула вперед, где брезжил какой-то свет."
    ...
    # смена фона снова на тоннель. МБ потом будет тоннель с дырой в потолке?
    uv "Он наверх вылез."
    ...
    uv "По веревке. Пошли по лесенке тогда."
    ...
    # засветка, потом заброшеный корпус изнутри, 
    $ meet('mt', 'Голос')
    mt "ШУ-У-У-УРИ-И-И-ИК!"
    "Снаружи раздался чей-то крик."
    ...
    # фон лесной дороги или старый корпус, мелкие спрайты пионеров в одном углу
    ...
    th "И как они теперь будут его искать? Наверное, придется нам прятаться обратно в тоннелях, или в убежище…"
    if alt_uvao_D5_hentai:
        dreamgirl "Ага-ага, спрятаться, и еще разок объездить кошечку. Ну, чтоб время скоротать."
    ...
    # мелкий шурик с дубиной в противоположном углу
    "И в этот момент из зарослей появился сам виновник торжества. Чумазый, ободраный, и все с той же приснопамятной железякой в руке."
    ...
    dreamgirl "Вот что значит советская педагогика! А если б ногами - вообще был бы как шелковый!"
    "Вожатая о чем-то спрашивала у Шурика, но тот явно не мог ничего объяснить, только растерянно озирался по сторонам."
    "Вскоре вся поисковая партия двинулась по направлению к лагерю. Шурика шатало туда-сюда, и шел он только благодаря поддержке Эла и Слави."
