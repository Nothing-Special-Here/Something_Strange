...
#
    ...
    menu:
        "Проигнорировать":
            ...
            # "Пропускать его совсем не хотелось, так что я поспешил подняться и поскорее одеться." - альт. вариант от Ленофага | + от Ravsii. Вполне годно.
            ...
            "Обед пропускать не хотелось, так что я поспешил подняться с кровати и поскорее одеться."   
...
#
    ...
    #у нас есть звук чирканья спички?| - Не поленился, нашёл - Ленофаг
    ...
    if alt_uvao_D5_sh_mines:
        ...
        scene bg black with fade2
        uv "Он где-то там."
        "Я услышал взмах. Наверное она показала в какую-то сторону своей ручкой, да вот только я ничего не видел."
        uv "Пошли."
...
#
    ...
    with dissolve
    window show
    th "Похоже, дальше копать не стали."
    "Впереди нас была балка, на которой висел фонарь, который, похоже, не излучал свет уже очень и очень давно."
    "Справа красовалась шахтёрская кирка."
    th "Видать кто-то забыл, либо ушёл и не вернулся."
    "Я бросил спичку, чтобы не обжечь пальцы, и все погрузилось во тьму…"
    #получится сделать затемнение, с подсветкой какого-нибудь угла ЦГ?|С этого места подробнее - Ленофаг
    "…но не полностью! Я заметил какой-то отсвет где-то за грудами выработанной породы."
    ...
    "Я воспользовался советом и стал осторожно приближаться к источнику света, стараясь не споткнуться и не шуметь. Юля кралась рядом."
    "Вскоре мы подобрались достаточно близко, чтобы выглянуть из-за камней."
    window hide
    scene cg d4_sh_sit
    window show
    ...
    th "Ну ты-то хоть с ума не сходи.{w} Да и вообще, даже если удастся его оглушить, как я его понесу потом?"
    dreamgirl "Мое дело предложить. Давай попробуем с ним поговорить тогда, вдруг он не совсем псих?"
    window hide
    scene bg int_mine_coalface
    with dissolve
    window show
    ...
    sh "Не притворяйся! Я все про тебя знаю! Ты - ненастоящий!"
    ...
    uv "Вставай. Не ушибся?"
    # Гоняем Шурика по шахтам, выгоняем в люк и вылезаем следом
    # Слышим крики ОД "[s]Лошааадкааааа![/s] Шуууурииииииик!"
    # Выглядываем из окна и видим сцену нейтрализации опасного психа.
    return
