...
#
    ...
    uv "Я точно не знаю. Там дверь, в дальнем конце. Только у меня не получается её открыть."
    me "Чем на месте стоять, пойдём, посмотрим? Вдруг вместе получится её открыть?"
    show uv grin with dspr
    uv "Пойдём, попробуем открыть!{w} Тогда нам скучно не будет!"
...
#    "Почему-то я решил не выдаваться в сравнительный анализ коитуса грызунов и людей."
    ...
    if alt_uvao_true:
        ...
        # Ravsii, ЦГшка Юли с городом готова, и залита на гит, вставь её сюда - Ленофаг
        "…"
        dreamgirl "…"
        ...
        th "Сейчас не время для шуток. То есть {b}это{/b}, по твоему, нормально?"
        dreamgirl "А я и не говорил, что это нормально. Ты думаешь Виола просто так попросила кошечку твою сфотографировать? Но, честно сказать, я тоже такого не ожидал."
        ...
        "Шкафчики с разными необходимыми припасами. Противогазы, аптечки с описью, инструменты… Не в целом, ничего такого, что могло бы пригодиться." 
...
#
    ...
    "Делать мне больше ничего не оставалось, так как впереди красовался тупик, и я прыгнул за ней."
    window hide
    scene bg int_mine 
    show uv normal at center 
    with dissolve
    window show
    "Я скатился по насыпи в шахту и, отряхнув шорты, майку и всё остальное, протянул руку в зияющую черноту. Нежные пальцы обвили мою ладонь и легко потащили за собой."
    uv "Здесь темно. Давай лапу."
    me "Руку."
    uv "А?"
    me "Мы зовём это рукой."
    uv "Как хочешь. Давай сюда, свою эту, руку."
    "Под ноги подворачивались камни и шпалы, я то и дело спотыкался, но Юля, будто не обращая внимания, вела меня дальше. Мы шли в полной тишине."
    window hide
    scene bg int_mine_crossroad
    show uv normal at center
    with dissolve
    window show
    "Наконец Юля остановилась и сказала:"
    uv "Направо."
    me "Я ведь ничего не вижу."
    show uv smile at center with dspr
    "Она усмехнулась."
    uv "Для этого и нужна ла…, ой, то есть рука."
    ...
    window show
    uv "Налево."
    "Так мы и ходили. То направо, то налево. Я не знаю, зачем она каждый раз говорила, куда мы поворачиваем, но я не был особо против."
    ...
    if alt_uvao_D5_sleep:
        ...
        uv "Кажется, недалеко от нас кто-то есть."
        th "Я догадываюсь, кто это может быть."
        uv "Похоже это твой друг из лагеря. Невысокий такой, в очках."
...
#
    ...
    uv "Он там, дальше."
    #Что-нибудь ещё.
    "Ничего не было видно. Я уже начинал чувствовать себя слепым."
    ...
    else: #По логике тут всё верно. Если мы не хентаим - то либо фоткаем и идём наверх, либо ищем конфеты, и, соответственно, спички
        th "У меня же спички в кармане лежат!"
        "Взяв из кармана заветный коробок, я достал из него спичку и зажёг её."
    "Спустя какое-то время мои глаза всё-же привыкли к небольшому огоньку, который маленькая спички пыталась удержать как можно дольше."
    ...
    #Дальше ЦГ испуганного шурика, а текст я даже пробовать не буду, ибо не знаю о чём писать.
