...
label alt_day5_uvao_hentai_sleep:
    ...
    me "У нас есть и мальчики, если они вас интересуют, но обычно девушки пользуются бОльшим спросом. К тому же мальчики более агрессивны, стараются поддерживать доминирующую роль в стае… В общем, лучше брать их отдельно, тогда всё пройдёт без эксцессов."
    ...
    "Пропускать еду совсем не хотелось, так что я поспешил подняться и поскорее одеться." 
...
label alt_day5_uvao_hentai_thinking:
    ...
    "Шорохи были подозрительно похожи на шаги, однако вскоре они затихли…{w} Но затем в дверь ударили!"
...
label alt_day5_uvao_thinking:
    ...
    "Юля деловито разворачивала конфеты одну за другой, иногда поглядывая на меня. Интересно, конфеты она тоже из столовой тягает?"
...
label alt_day5_uvao_mines_begin:
    ...
    me "Юля, ты там, внизу?"
    uv "Ну да. Прыгай!"
    "Я поднял голову. Насколько я мог разглядеть, дальше за провалом потолок обвалился, да и перепрыгнуть яму мне всё равно вряд ли бы удалось."
...
label alt_day5_uvao_from_mines_to_dinner:
    ...
    hide uv with dissolve
    "Щуря отвыкшие от света глаза, я подошёл ближе. Юля тем временем забралась по лестнице куда-то наверх, немного повозилась там, чем-то позвякивая и спустилась вниз."
    show uv normal close at center with dissolve
