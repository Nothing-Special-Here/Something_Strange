...
label alt_day5_uvao_road_to_old_camp:
    ...
    window show
    # Есть мысля вставить сюда No tresspassing из остов
    ...
    "Юля проворно скользнула мимо меня и направилась мимо лестницы."
...
label alt_day5_uvao_bunker:
    ...
    dreamgirl "Сиська вполне человеческая и весьма упругая, кстати. Да и все остальное, кроме хвоста и ушей - тоже вполне обычное."
...
label alt_day5_uvao_hentai_sleep:
    ...
    me "У нас есть и мальчики, если они вас интересуют, но обычно девушки пользуются бОльшим спросом. К тому же мальчики более агрессивны, стараются поддерживать доминирующую роль в стае… В общем, лучше брать их отдельно, тогда все пройдёт без эксцессов."
...
label alt_day5_uvao_thinking:
    ...
    th "Все, водопад безумных идей иссяк? Какие будут {i}реальные{/i} предложения?"
...
label alt_day5_uvao_mines_begin:
    "Затвор был в точности такой же, как и у первой двери. Указав Юле на кнопку, я приготовился крутить штурвал."
    play sound sfx_open_door_mines_metal
    "Механизм шёл туже, чем у первой двери, но все же скоро сдался. Я навалился всем телом на дверь и она неохотно повернулась на заржавевших петлях."
    window hide
    $ night_time()
    $ persistent.sprite_time = "night"
    scene bg int_catacombs_entrance
    ...
    if alt_uvao_D5_hentai:
        ...
        "Стоило мне только подумать о Виоле, как сразу по всеми телу побежали мурашки."
...
label alt_day5_parking_back:
    ...
    window hide
    scene bg ext_washstand_day with fade # Дневные умывальники разительно контрастируют с вечерней площадью.Думаю, исполню в ФШ вечернюю версию. Наложу розоватый фильтр, например. Напомните мне только, забыть могу.
    show mt angry panama pioneer at fleft
    with dissolve
